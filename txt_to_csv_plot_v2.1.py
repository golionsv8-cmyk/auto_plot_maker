...
class MainWindow(QMainWindow):
    def clear_axis_ranges(self):
        ...
        if self.last_series is not None:
            try:
                ...
                QMessageBox.critical(self, "Axis Range Error", str(exc))
    ...
    def _build_plot_group(self) -> QGroupBox:
        ...
        self.cmd_lw_spin = QDoubleSpinBox()
        self.cmd_lw_spin.setRange(0.1, 10.0)
        self.cmd_lw_spin.setSingleStep(0.1)
        self.cmd_lw_spin.setValue(1.5)

        self.rpm_lw_spin = QDoubleSpinBox()
        self.rpm_lw_spin.setRange(0.1, 10.0)
        self.rpm_lw_spin.setSingleStep(0.1)
        self.rpm_lw_spin.setValue(1.2)

        self.secondary_lw_spin = QDoubleSpinBox()
        self.secondary_lw_spin.setRange(0.1, 10.0)
        self.secondary_lw_spin.setSingleStep(0.1)
        self.secondary_lw_spin.setValue(0.9)

        self.ratio_w_spin = QDoubleSpinBox()
        self.ratio_w_spin.setRange(0.5, 10.0)
        self.ratio_w_spin.setSingleStep(0.1)
        self.ratio_w_spin.setValue(DEFAULT_RATIO_W)

        self.ratio_h_spin = QDoubleSpinBox()
        self.ratio_h_spin.setRange(0.5, 10.0)
        self.ratio_h_spin.setSingleStep(0.1)
        self.ratio_h_spin.setValue(DEFAULT_RATIO_H)

        self.title_pad_spin = QDoubleSpinBox()
        self.title_pad_spin.setRange(0.0, 60.0)
        self.title_pad_spin.setSingleStep(1.0)
        self.title_pad_spin.setValue(DEFAULT_TITLE_PAD)
    ...
    def run_from_txt(self):
        ...
        if not txt_path:
            QMessageBox.warning(self, "Missing TXT", "Select Data File!!")
            return
        if not Path(txt_path).is_file():
            QMessageBox.warning(self, "Invalid TXT", "Invalid Data File!!")
            return
        if not csv_save_path:
            QMessageBox.warning(self, "Missing CSV path", "Selec CSV Save Path!!")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", f"Error Occur During data converting!!\n\n{exc}")
            self.set_status(f"Error: {exc}")

    def run_from_csv(self):
        csv_path = self.csv_load_edit.text().strip()
        if not csv_path:
            QMessageBox.warning(self, "Missing CSV", "Select CSV File!!")
            return
        if not Path(csv_path).is_file():
            QMessageBox.warning(self, "Invalid CSV", "Invalid CSV File!!")
            return
        ...
        try:
            ...
            QMessageBox.critical(self, "Error", f"Error Occur During Plot Data!! \n\n{exc}")
            self.set_status(f"Error: {exc}")
    ...
    def save_single_figure(self, fig_no: int):
        ...
        if fig is None:
            QMessageBox.information(self, "No Figure", "Draw Figure First!!")
            return
